...
import { FiSearch, FiExternalLink, FiTrash2, FiCopy, FiArrowLeft } from 'react-icons/fi';
...
const ArchivedMessages = () => {
  ...
  const {
    ...
    queryFn: ({ pageParam }) => api.getArchivedMessages({
      cursor: pageParam,
      limit: 30,
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: 'Arşiv silindi', status: 'success', position: 'top', duration: 1500 });
    },
    onError: (error) => {
      toast({
        title: 'Hata',
        ...
        {isLoading ? (
          ...
          <VStack align="stretch" spacing="4">
            {archives.map((a) => {
              ...
              const channelName = channel?.name || a.channelName || a.channelId || 'Kanal';
              const messageText = a.messageContent?.text || a.content || a.text || (a.message?.text) || '';
              const derivedChannelId = a.channelId || channel?._id || channel?.id;
              const derivedMessageId = a.message?._id || a.message?.id || a.messageId;
              return (
                <Box key={id} border="1px solid" borderColor="gray.100" p="3" borderRadius="md">
                  ...
                  <Text fontSize="sm" color="gray.700">{messageText || '(Metin içeriği yok)'}</Text>
                </Box>
              );
            })}
            {hasNextPage && (
