...
import {PolicyTypeLabel, policyTypes} from '../../../config';
...
const EditPolicy = ({id}) => {
  ...
  const {mutateAsync, isPending} = useMutation({
    mutationFn: values =>
      isNew ? api.createPolicy(values) : api.updatePolicy(id, values),
  });
            ...
            <FormControl isInvalid={!!errors.type} mb="4">
              ...
              <Select
                ...
                {policyTypes.map(type => {
                  return <option value={type}>{PolicyTypeLabel[type]}</option>;
                })}
              </Select>
              <FormErrorMessage>{errors.type?.message}</FormErrorMessage>
              <FormHelperText>
                Her türden sadece bir tane sözleşme olabilir. Var olan bir tür
                seçtiğinizde öncekinin üzerine yazar.
              </FormHelperText>
            </FormControl>
